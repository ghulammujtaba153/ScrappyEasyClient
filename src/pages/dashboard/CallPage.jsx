...
import { MdCall, MdWarning, MdArrowForward } from "react-icons/md";
...
const CallPage = () => {
    ...
    const [hasTwilio, setHasTwilio] = useState(true);
    ...
    useEffect(() => {
        const checkConfig = () => {
            ...
            if (saved) {
                ...
                if (!config.accountSid || !config.phoneNumber) {
                    setHasTwilio(false);
                }
            } else {
                setHasTwilio(false);
    ...
    const handleStartCall = async (e) => {
        ...
        try {
            const saved = localStorage.getItem("twilio_config");
            const twilioConfig = saved ? JSON.parse(saved) : null;
        ...
        <div className="p-6 relative min-h-[80vh] bg-gray-50 uppercase tracking-tight">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="flex flex-col gap-6">
                    {!hasTwilio && (
                        <div className="bg-amber-50 border border-amber-200 p-6 rounded-2xl flex flex-col gap-4">
                            <div className="flex items-center gap-3 text-amber-800">
                                <MdWarning size={24} />
                                <h3 className="font-black uppercase italic">Twilio Not Configured</h3>
                            </div>
                            <p className="text-xs font-bold text-amber-700 leading-relaxed uppercase">
                                You need to connect your Twilio account before you can launch AI scheduling calls.
                            </p>
                            <Link
                                to="/dashboard/twilio-settings"
                                className="inline-flex items-center gap-2 text-xs font-black text-white bg-amber-600 hover:bg-amber-700 px-4 py-2 rounded-lg transition-all w-fit uppercase"
                            >
                                Configure Now <MdArrowForward />
                            </Link>
                        </div>
                    )}
